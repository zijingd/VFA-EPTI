...
%
%
function fitRes = mwi_2T13T2s_cx_VFAT2s_EPTI(algoPara,imgPara)
...
[~,rho0] = DESPOT1(mean(abs(s(:,1:2)),2),fa,tr,'b1',b1);
% estimate t1 from later echo
% [t10,~] = DESPOT1(abs(s(:,end-3)),fa,tr,'b1',b1);
[t10,~] = DESPOT1(mean(abs(s(:,2:5)),2),fa,tr,'b1',b1); % zijing
...
if numMagn==numel(te) % magnitude fitting
    ...
    totalField0 = db0(:).';  totalFieldlb   = db0(:).'-50;                 totalFieldub    = db0(:).'+50;
